...
| `io.ktor:ktor-client-okhttp` | Android engine | Keep on Android, add `io.ktor:ktor-client-darwin` 3.6.0 for iOS, engine chosen through DI. From B4 both can arrive through `io.ktor:ktor-client-engine-defaults` 3.6.0, a code-free aggregator whose Gradle Module Metadata maps JVM → okhttp and Apple targets → darwin (it publishes no Android variant, so Android resolves the JVM one) | A5 / B4 / C2 |
