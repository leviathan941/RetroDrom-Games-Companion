...
| `io.coil-kt.coil3:coil-network-ktor3` | 3.6.3 | yes | Replaced `coil-network-okhttp` in A2. Since A5 it takes the app's injected base `HttpClient`, so it follows whatever engine `ktor-client-engine-defaults` resolves per target. |
| `com.mikepenz:aboutlibraries-compose-m3` | 15.2.0 | yes | |
| `io.ktor:ktor-client-*` (core, content-negotiation, engine-defaults, resources, serialization) | 3.6.0 | yes | Engine is the only platform part, and since A5 it is resolved rather than named — see ADR-0005. |
...
| `io.ktor:ktor-client-okhttp` | Android engine | **Done in A5.** No longer declared: `:network` depends on `io.ktor:ktor-client-engine-defaults` 3.6.0, a code-free aggregator whose Gradle Module Metadata maps JVM → `ktor-client-okhttp` and Apple targets → `ktor-client-darwin` (it publishes no Android variant, so Android resolves the JVM one, as `ktor-client-core` already does). DI provides a base `HttpClient`, since the aggregator's engine is only reachable through the no-arg `HttpClient()`. `org.slf4j:slf4j-api` still arrives with the OkHttp engine, as before. | A5 (ADR-0005) |
...
- Metro is a Kotlin **compiler plugin**. A3 verified that its Gradle plugin hooks correctly into
  AGP-supplied Kotlin compilation: AGP 9's built-in Kotlin runs every
  `KotlinCompilerPluginSupportPlugin` the same way KGP does. No Kotlin Gradle plugin is needed —
  and none may be applied, since AGP 9 hard-fails on `kotlin.android`, `kapt` and
  `kotlin.multiplatform` alongside built-in Kotlin.
